...
function aboutPage() {
  ...
  let aboutMeDiv = $(`<div id='aboutMeDiv'>
        ...
        I served in the army as a fire fighter for 3 years.</br>
        Currently working in Zap-Group for the past 3 years.</br>
        Love to play video Games and watch football.</br>
        </p>
    ...
    success: function (result) {
      ...
      var options = {
        ...
        subtitles: [
          {
            text:
              "Here you can see the coins you choose and their current value in charts ",
          },
        ],
        axisX: {
          title: "COINS",
...
function failFunc() {
  ...
  alert(
    "Somthing went wrong getting the details. Please check your spelling or coin not exists"
  );
}
...
function getFromServer(coinId) {
  $.ajax({
    ...
    success: function (oneCoinFromServer) {
      ...
      $(`#newInfoDiv${coinId}`).html(
        "USD &#xf155; : " +
          moreInfoCoin.market_data.current_price.usd +
          "$</br></br> Euro &#xf153; : " +
          moreInfoCoin.market_data.current_price.eur +
          "</br></br> NIS &#xf20b; : " +
          moreInfoCoin.market_data.current_price.ils +
          "</br></br>"
      );
      let coinImg = $("<img>");
...
function getSearch(e) {
  ...
  let searchInputValue = document
    .querySelector("#coinSearchInput")
    .value.toLowerCase();
  let allCardsDiv = document.querySelectorAll(".coinDivClass");
  for (let i = 0; i < allCardsDiv.length; i++) {
    if (allCardsDiv[i].id == searchInputValue) {
      hideAllDiv();
      allCardsDiv[i].style.display = "inline-block";
    }
  }
  if (searchInputValue === "") {
    showAllDiv();
...
function hideAllDiv() {
  let allCardsDiv = document.querySelectorAll(".coinDivClass");
  for (let i = 0; i < allCardsDiv.length; i++) {
    allCardsDiv[i].style.display = "none";
  }
}
